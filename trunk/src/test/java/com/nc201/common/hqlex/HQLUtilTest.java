...
public class HQLUtilTest {
	...
	public void testTrans2Sql() throws Exception {
		...
		DetachedSessionFactoryImpl.getInstance().pushClass(userJson);

		DetachedSessionFactoryImpl.getInstance().pushClass(bookJson);

		DetachedSessionFactoryImpl.getInstance().pushClass(userClassJson);
		...
		for (Iterator iterator = list.iterator(); iterator.hasNext();) {
			...
			for (int i = 0; i < dbTypes.length; i++) {
				...
				DetachedSessionFactoryImpl.getInstance().setDialect(dbTypes[i]);
				result = HQLUtil.trans2Sql(hql);

				String dbType = DetachedSessionFactoryImpl.getInstance()
						.getDbName();
		...
		HQLUtil.trans2Sql("select new map( u.name as name2,u.age as age2) from user u ");
		HQLUtil.trans2Sql("select u from book u where u.user.name is not null");
